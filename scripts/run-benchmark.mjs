// Run the full benchmark against the configured Ollama (external at 192.168.4.2)
...
async function main() {
  ...
  console.log('1. Fetching model metadata...');
  const meta = await fetchJSON('/api/show', { name: model });
  console.log(`   Context: ${meta.context_length?.toLocaleString() || 'unknown'}`);
  console.log(`   Parameters: ${meta.details?.parameter_size || 'unknown'}`);
  console.log(`   Quantization: ${meta.details?.quantization_level || 'unknown'}`);
  console.log('');

  // 2. Basic generation
  console.log('2. Basic generation (1K context)...');
  const r1 = await testGenerate("Say 'benchmark test' and stop.", 1024, 60000);
  if (r1.success) {
  ...
  // 4. Throughput
  console.log('4. Throughput tests...');
  for (const ctx of [1024, 4096, 8192, 16384, 32768, 65536]) {
    ...
    if (result.success && result.evalDuration > 0) {
      ...
      console.log(`${tps.toFixed(2).padStart(7)} tok/s (${result.tokens} out, ${firstMs.toFixed(0).padStart(5)}ms first token)`);
    } else {
      console.log(`FAIL: ${result.error?.substring(0, 50)}`);
    }
  }
  ...
  // 5. Memory test
  if (maxWorking >= 8192) {
    console.log('5. Memory retention (needle at 50%)...');
    ...
    const prompt = contextText + "\n\nWhat is the code? Answer with just the code.";
