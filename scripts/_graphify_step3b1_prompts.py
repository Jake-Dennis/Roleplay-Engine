"""Step 3B1: Generate chunk prompt files for subagent dispatch."""
import json
from pathlib import Path

chunks_meta = json.loads(Path('graphify-out/.graphify_chunks_meta.json').read_text(encoding='utf-8'))
total = chunks_meta['total_chunks']
chunks_dir = Path('graphify-out/.graphify_chunks')

# The extraction spec template (verbatim from references/extraction-spec.md)
TEMPLATE = '''You are a graphify extraction subagent. Read the files listed and extract a knowledge graph fragment.
...
prompts_dir = Path('graphify-out/.graphify_chunk_prompts')
prompts_dir.mkdir(parents=True, exist_ok=True)
for i in range(1, total + 1):
    files = (chunks_dir / f'chunk_{i:02d}.txt').read_text(encoding='utf-8').splitlines()
    file_list = '\n'.join(files)
    chunk_path = f'C:\\Users\\JakeP\\Documents\\GitHub\\Roleplay-Engine\\graphify-out\\.graphify_chunk_{i:02d}.json'
    prompt = TEMPLATE.replace('{CHUNK_NUM}', str(i)).replace('{TOTAL_CHUNKS}', str(total)).replace('{FILE_LIST}', file_list).replace('{CHUNK_PATH}', chunk_path)
    (prompts_dir / f'prompt_{i:02d}.txt').write_text(prompt, encoding='utf-8')
print(f'Wrote {total} prompts to {prompts_dir}')
print(f'Avg prompt size: {sum((prompts_dir / f"prompt_{i:02d}.txt").stat().st_size for i in range(1, total+1)) // total} bytes')
