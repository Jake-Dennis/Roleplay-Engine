...
- [ ] T6: `python -c "import os.path; assert os.path.exists(r'src/app/(app)/admin/restructure/page.tsx')"` — admin page exists
- [ ] T7: `python -c "import os; assert os.path.getsize(r'src/components/wiki/frontmatter-properties-panel.tsx') > 9000"` — dormant UI added
- [ ] T8: `bun test src/lib/jobs/__tests__/wiki-restructure-suggestions.test.ts` — restructure job handler works
