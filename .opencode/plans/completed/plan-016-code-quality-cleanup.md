...
## Verification
- [ ] 1a: `powershell -NoProfile -Command "if ((Select-String -Path src/lib/wiki/bulk-move.ts -Pattern 'err: unknown' -SimpleMatch) -and (Select-String -Path src/lib/wiki/bulk-recategorize.ts -Pattern 'err: unknown' -SimpleMatch) -and (Select-String -Path src/app/api/wiki/merge/route.ts -Pattern 'err: unknown' -SimpleMatch)) { exit 0 } else { exit 1 }"` — should exit 0 (4 any→unknown replacements)
- [ ] 1b: `powershell -NoProfile -Command "if ((Select-String -Path src/components/wiki/markdown-editor.tsx -Pattern 'useMemo' -SimpleMatch) -and (Select-String -Path src/components/wiki/wiki-quick-switcher.tsx -Pattern 'setTimeout' -SimpleMatch)) { exit 0 } else { exit 1 }"` — should exit 0 (useMemo and setTimeout patterns applied)
- [ ] 1c: `powershell -NoProfile -Command "if (-not (Select-String -Path src/lib/ollama.ts -Pattern 'require' -SimpleMatch)) { exit 0 } else { exit 1 }"` — should exit 0 (no require() in ollama.ts)
- [ ] 1d: `powershell -NoProfile -Command "if ((Select-String -Path src/lib/message-summarizer.ts -Pattern '@deprecated' -SimpleMatch) -and (Select-String -Path src/lib/api-client.ts -Pattern '@deprecated' -SimpleMatch)) { exit 0 } else { exit 1 }"` — should exit 0 (deprecated markers present)
- [ ] 1e: `powershell -NoProfile -Command "if (Select-String -Path eslint.config.mjs -Pattern 'docs/historical-evidence' -SimpleMatch) { exit 0 } else { exit 1 }"` — should exit 0 (historical-evidence excluded from lint)
- [ ] 2a: `powershell -NoProfile -Command "if (-not (Select-String -Path src/app/api/benchmark/roleplay/route.ts -Pattern 'unauthorizedError' -SimpleMatch)) { exit 0 } else { exit 1 }"` — should exit 0 (unauthorizedError import removed)
