...
export async function GET(
  ...
  const npc = db.prepare(
    "SELECT id, user_id, name, file_path, canon_tier, canon_layer, location_id, importance, tags, created_at FROM npcs WHERE id = ? AND user_id = ?"
  ).get(id, decoded.sub);
...
export async function PUT(
  ...
  const existing = db.prepare("SELECT * FROM npcs WHERE id = ? AND user_id = ?").get(id, decoded.sub) as any;
  ...
  if (newFilePath !== existing.file_path) deleteLoreFile(decoded.sub, existing.file_path);
  writeLoreFile(decoded.sub, "npcs", newFilePath, mdContent);
...
export async function DELETE(
  ...
  const existing = db.prepare("SELECT file_path FROM npcs WHERE id = ? AND user_id = ?").get(id, decoded.sub) as { file_path: string } | undefined;
  if (!existing) return NextResponse.json({ error: "NPC not found" }, { status: 404 });
