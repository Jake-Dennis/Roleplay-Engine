...
/**
 * GET /api/npcs/{id}
 * Gets a single NPC by entity_id (e.g. "npc:uuid") or by name.
 * Reads from wiki pages with type=entity and subtype=character.
 *
 * @param request - The incoming Next.js request object
 * @param params - Route parameters containing the NPC entity_id
 * @returns NextResponse with { npc }
 * @throws 401 - If authentication fails
 * @throws 404 - If NPC not found
 */
...
const { id } = await params;
  ...
  try {
    ...
    const npc = {
      ...
      entityId: fm.entity_id || null,
      name: fm.title,
      description: character.content || null,
      universeId: fm.universe || null,
