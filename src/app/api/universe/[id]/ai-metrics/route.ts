...
// Simple token estimate (mirrors estimateTokens in prompt-builder.ts)
function estimateTokens(text: string): number {
  return Math.ceil(text.length / 4);
}

/**
 * GET /api/universe/[id]/ai-metrics
 *
 * Returns context window usage data, model info, and wiki/session stats
 * for a given universe.
 *
 * @param request - The incoming Next.js request object
 * @param params - Route parameters containing `{ id }` — the universe UUID
 * @returns NextResponse with universe context metrics, model info, and stats
 * @throws 401 - If authentication fails
 * @throws 403 - If user does not have access to the universe
